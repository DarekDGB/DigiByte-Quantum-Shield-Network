...
from dqsnetwork.v4.trust_profile import FIPS204_ML_DSA_65_PROFILE
...
def test_v48e_oqs_mldsa_backend_rejects_empty_signature_bytes() -> None:
    ...
    with pytest.raises(DqsnV4RealCryptoBackendError, match="signature"):
        backend.sign_message(algorithm="ml-dsa", private_key_reference="hsm://dqsn/ml-dsa/v1", message=b"message")
...
def test_v48g_dqsn_oqs_mldsa_backend_wraps_import_resolver_constructor_and_rejects_non_bool_verify(
    ...
    with pytest.raises(DqsnV4RealCryptoBackendError, match="import failed closed") as import_error:
        _ = backend.backend_version
    ...
    with pytest.raises(DqsnV4RealCryptoBackendError, match="sign failed closed") as constructor_error:
        backend.sign_message(algorithm="ml-dsa", private_key_reference="hsm://dqsn/ml-dsa/v1", message=b"message")
...
def test_v48g_dqsn_oqs_mldsa_backend_rejects_direct_test_only_private_reference() -> None:
    backend = OqsMlDsaBackend(private_key_resolver=resolver, oqs_module=FakeOqsModule())
